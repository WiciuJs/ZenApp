...
import { faCalendar, faAddressBook, faBox, faGift, faSpa, faRightFromBracket } from '@fortawesome/free-solid-svg-icons';
            ...
            <li key={item.path} className={`${styles.navItem} ${location.pathname === item.path ? styles.active : ''}`}>
              <Link to={item.path} onClick={item.onClick}>
                <div className={`${styles.iconContainer}`}>
                  <FontAwesomeIcon className={`${styles.navIcon}`} icon={item.icon} />
                </div>
              </Link>
            </li>
          ))}
